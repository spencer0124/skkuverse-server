...
// exclude-reasons.json shape: { <reasonKey>: { ko: string, en?: string } }.
// Generated by the crawler's codegen from the root exclude-reasons.json SSOT,
// so reason copy is server-driven — a new key needs no app release.
type ExcludeReasonMap = Record<string, { ko: string; en?: string }>;
...
export class TabConfigService {
  ...
  load(): void {
    ...
    const rawReasons = this.loadJSON("exclude-reasons.json");
    ...
    if (
      ...
      this.fatal("exclude-reasons.json must be a JSON object (key → label map)");
    }

    const categoriesArr = rawCategories as CategoryConfig[];
    const sourcesArr = rawSources as SourceConfig[];
    const reasonMap = rawReasons as ExcludeReasonMap;

    // Both files come from the same codegen run; a source referencing a key
    // the map doesn't know means the copy step was skipped or partial.
    for (const s of sourcesArr) {
      if (s.excludeReason != null && !(s.excludeReason in reasonMap)) {
        this.fatal(
          `sources.json: "${s.id}" has excludeReason "${s.excludeReason}" ` +
            "with no entry in exclude-reasons.json",
        );
      }
    }
    ...
    this._responseByLang = Object.freeze({
      ko: Object.freeze(
        this.buildTabsResponse(categoriesArr, sourceMap, reasonMap, "ko"),
      ),
      en: Object.freeze(
        this.buildTabsResponse(categoriesArr, sourceMap, reasonMap, "en"),
      ),
    });
    ...
    for (const cat of categoriesArr) {
      ...
      if (cat.tabMode === "fixed") {
        ...
        for (const id of cat.sourceIds) {
          ...
          if (!source) continue;
          const reason = source.excludeReason ?? null;
          const reasonLabel = reason ? reasonMap[reason] : undefined;
          sources.push({
            id: source.id,
            name: source.name,
            campus: source.campus ?? null,
            ...
            // Same lang fallback as tab labels: requested → en → ko.
            excludeReasonText: reasonLabel
              ? reasonLabel[lang] || reasonLabel.en || reasonLabel.ko
              : null,
          });
        }
