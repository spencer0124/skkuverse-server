...
5 endpoints: `GET /notices/tabs` (server-driven tab config), `GET /notices/source/:sourceId` (single dept list), `GET /notices` (multi-dept merged list), `GET /notices/:deptId/:articleNo` (detail), `GET /notices/proxy/attachment` (file proxy).

- **SSOT files**: `sources.json` (147 departments) and `categories.json` (9 tab definitions) — both generated by skkuverse-crawler and loaded at startup with fail-fast validation (`tabConfig.js`).
