...
- **Timezone**: Bus and notices date/time logic uses `moment-timezone` with `Asia/Seoul`. **The event map is deliberately timezone-free** — every opening window in `MapPlaceDoc.hours` is a pair of absolute instants, so no server-side KST conversion is ever needed (rationale in `src/map/map-places.types.ts`). Don't "fix" it by adding one.
