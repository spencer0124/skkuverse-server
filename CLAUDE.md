...
Requires `.env` with: MongoDB connection (`MONGO_URL`, `MONGO_DB_NAME_*`, `MONGO_AD_DB_NAME`), bus API endpoints (`API_HSSC_*_PROD`, `API_HSSC_*_DEV`, `API_JONGRO*_PROD`, `API_JONGRO*_DEV`, `API_STATION_*`), Firebase credentials (`FIREBASE_SERVICE_ACCOUNT`), and app config (`APP_MIN_VERSION`, `APP_LATEST_VERSION`, `APP_UPDATE_URL`). See `lib/config.js` for the full list. `NODE_ENV` and `USE_PROD_API` are set per execution context (CLI/Docker), not in `.env`.
