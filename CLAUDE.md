...
5 endpoints: `GET /notices/tabs` (server-driven tab config), `GET /notices/dept/:deptId` (single dept list), `GET /notices` (multi-dept merged list), `GET /notices/:deptId/:articleNo` (detail), `GET /notices/proxy/attachment` (file proxy).

- **SSOT files**: `departments.json` (147 departments) and `categories.json` (9 tab definitions) — both generated by skkuverse-crawler and loaded at startup with fail-fast validation (`tabConfig.js`).
- **Tab config** (`tabConfig.js`): Validates JSON structure at startup (`process.exit(1)` on failure), pre-computes per-language responses (`Object.freeze`). Tagged payload pattern: `tabMode: "fixed"` → `fixed: { deptId, name, campus }`, `tabMode: "picker"` → `picker: { departments, maxSelection, defaultDeptIds }`.
- **Architecture doc**: `docs/notices-api-architecture.md` — full design decisions, endpoint specs, and incident retrospective.
...
- **Config** (`lib/config.js`): Centralized env var loading with environment separation. `NODE_ENV` controls DB suffix (`_dev`/`_test`/none), `USE_PROD_API` controls API endpoint selection independently. Required values validated at startup (process.exit(1) if missing, skipped in test mode). Same fail-fast pattern is used by `features/notices/tabConfig.js` for JSON config validation.
