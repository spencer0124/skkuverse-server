...
Behind Nginx with TLS via Cloudflare. Two origin hosts — the OCI Free Tier VM and a rented Naver host — serve `api.skkuverse.com` together behind a Cloudflare load balancer, weighted per host ([docs/decisions/0009-multi-origin-active-active.md](docs/decisions/0009-multi-origin-active-active.md), [docs/how-to/operate-load-balancer.md](docs/how-to/operate-load-balancer.md)). Deployed on push to `main` by `.github/workflows/deploy.yml`, one origin host at a time (the per-host steps are `.github/workflows/deploy-host.yml`); the rented host, which GitHub's runners cannot reach, then deploys itself from a self-hosted runner via `.github/workflows/deploy-mnemosyne.yml` ([docs/cicd-and-branch-protection.md](docs/cicd-and-branch-protection.md#deploy-mnemosyne-self-hosted-runner)). Only the host whose `/etc/skkuverse/host.env` says `POLLER_ROLE=active` runs the poller ([docs/how-to/fail-over-poller.md](docs/how-to/fail-over-poller.md)). The origin accepts HTTP(S) only from Cloudflare ([docs/how-to/lock-origin-to-cloudflare.md](docs/how-to/lock-origin-to-cloudflare.md)); how production is watched and alerted on: [docs/how-to/monitor-production.md](docs/how-to/monitor-production.md).
